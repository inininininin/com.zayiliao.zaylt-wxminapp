...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          for (var i = 0; i < that.data.patients.length; i++) {
            if (that.data.patients[i].id == patient_id) {
              that.data.patients[i].status = '4'
            }
          }
          that.setData({
  ...
  onLoad: function (options) {
    ...
    var userToken = wx.getStorageSync("userToken");
    wx.request({
      url: 'https://www.njshangka.com/zaylt/c/h/e/clinicinfo/clinichome',
      method: 'post',
      data: {
        id: clinicId,
        token: userToken,
